...
# Apply button
    ...
    def add_row_to_table(self, table, text):
        try:
            ...
            # --- NEW: Use the get_event_severity method for consistency ---
            # --- UPGRADED: Threat Engine Priority Coloring ---
            e_type = event.get("type", "")
            ...
            # 2. Fallback to default Event Type coloring if Threat Engine didn't flag it
                ...
                severity = "High"
                bg_color = QColor("#490202")  
                text_color = QColor("#ff7b72") 
            elif e_type == "NETWORK_CONNECTION":
                severity = "Medium"
                bg_color = QColor("#0a3069")  
                text_color = QColor("#79c0ff") 
            elif e_type in ["USER_SESSION_STARTED", "USER_SESSION_ENDED", "SYSTEM_BOOT_INFO"]:
                severity = "Info"
                bg_color = QColor("#1e0a3c")  
                text_color = QColor("#a371f7") 
            else:
                severity = "Low"
                bg_color = QColor("#0d1117")  
                text_color = QColor("#c9d1d9")
                if e_type == "UAC_DETECTED": 
                    bg_color = QColor("#3d3301") 
                    text_color = QColor("#e3b341")
            # --------------------------------------------------
